...
<body class="bg-gray-50 min-h-screen text-gray-800">
    ...
    <nav class="sticky top-0 z-50 glass px-6 py-4 mb-8">
        <div class="max-w-6xl mx-auto flex justify-between items-center">
            ...
            <div>
                <a href="index.php"
                    class="text-sm font-medium text-gray-600 hover:text-primary transition-colors flex items-center gap-2">
        ...
        <div class="flex flex-col md:flex-row justify-between items-end mb-8 gap-4 animate-fade-in">
            ...
            <div class="flex gap-2">
                <?php foreach ($countByType as $stat): ?>
                    <div
                        class="bg-white/80 backdrop-blur px-3 py-1 rounded-lg border border-gray-100 shadow-sm text-xs font-medium text-gray-600">
                        <span class="text-primary font-bold"><?= $stat['count'] ?></span>
                        <?= htmlspecialchars($stat['tipologia']) ?>
                    </div>
                <?php endforeach; ?>
                <div
                    class="bg-primary/10 px-3 py-1 rounded-lg border border-primary/20 shadow-sm text-xs font-medium text-primary">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-4">
                <div class="glass-card sticky top-24 animate-fade-in delay-100">
                    <h2 class="text-lg font-bold text-primary mb-4 border-b border-gray-100 pb-2">➕ Aggiungi Medicinale
                    </h2>
                    <form id="add-medicine-form" class="space-y-4">
                        <div class="space-y-1">
                            <label class="text-xs font-medium text-gray-700">Nome Medicinale *</label>
                            <input type="text" name="nome"
                                class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm"
                                required placeholder="Es: Arnica Montana">
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-1">
                                ...
                                <select name="tipologia"
                                    class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm bg-white"
                                    required>
                            ...
                            <div class="space-y-1">
                                ...
                                <input type="text" name="formato"
                                    class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm"
                                    placeholder="Es: Gocce">
                            </div>
                        </div>

                        <div class="space-y-1">
                            <label class="text-xs font-medium text-gray-700">Dosaggio Standard</label>
                            <input type="text" name="dosaggio_standard"
                                class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm"
                                placeholder="Es: 5g 3x/die">
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <textarea name="note"
                                class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm"
                                rows="2" placeholder="Indicazioni opzionali..."></textarea>
                        </div>

                        <button type="submit"
                            class="w-full bg-primary text-white font-bold py-2 rounded-lg shadow-md hover:bg-green-700 transition-colors">Salva
                            Medicinale</button>
                    </form>
                </div>
            </div>
            ...
            <div class="lg:col-span-8">
                ...
                <div class="glass flex flex-col md:flex-row gap-4 p-4 rounded-xl mb-6 animate-fade-in delay-200">
                    <div class="flex-1">
                        <input type="text" id="search-medicine"
                            class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm bg-white/50"
                ...
                <div class="glass-card animate-fade-in delay-300 min-h-[500px]">
                    <h3 class="text-lg font-bold text-gray-800 mb-4 pl-2 border-l-4 border-secondary">Elenco Prodotti
                    </h3>
                                ...
                                <?php foreach ($medicines as $medicine): ?>
                                    <tr class="medicine-row hover:bg-gray-50/50 transition-colors group"
                                        ...
                                        <td class="px-4 py-3 font-medium text-gray-900">
                                            <?= htmlspecialchars($medicine['nome']) ?></td>
                                        <td class="px-4 py-3 text-gray-600"><?= htmlspecialchars($medicine['tipologia']) ?>
                                        </td>
                                        <td class="px-4 py-3 text-gray-500">
                                            <?= htmlspecialchars($medicine['formato'] ?? '-') ?></td>
                                        <td class="px-4 py-3 text-gray-500 truncate max-w-[150px]"
                                            title="<?= htmlspecialchars($medicine['dosaggio_standard'] ?? '') ?>">
                                            <?= htmlspecialchars($medicine['dosaggio_standard'] ?? '-') ?></td>
                                        <td class="px-4 py-3 text-center">
                                            <?php if ($medicine['attivo']): ?>
                                                <span
                                                    class="px-2 py-1 rounded-full bg-green-100 text-green-700 text-xs font-bold">Attivo</span>
                                            <?php else: ?>
                                                <span
                                                    class="px-2 py-1 rounded-full bg-red-100 text-red-700 text-xs font-bold">Inattivo</span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="px-4 py-3 text-right flex justify-end gap-2">
                                            ...
                                            <?php if ($medicine['attivo']): ?>
                                                <button onclick="toggleMedicine(<?= $medicine['id'] ?>, 0)"
                                            ...
                                            <?php else: ?>
                                                <button onclick="toggleMedicine(<?= $medicine['id'] ?>, 1)"
